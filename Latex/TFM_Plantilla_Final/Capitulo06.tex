...
El token de acceso contiene \texttt{typ}, \texttt{sub}, \texttt{accountId}, \texttt{deviceId}, plan, entitlements, \texttt{jti} y expiración. Fase~2 añade los roles. El token de reproducción sustituye plan y permisos generales por \texttt{assetId}, \texttt{sessionId}, \texttt{clientInstanceId} y los ámbitos \texttt{manifest:read}, \texttt{content:read}, \texttt{license:request} y \texttt{heartbeat:write}. Los secretos de acceso y reproducción son distintos para impedir que una credencial de un tipo pueda validarse con la clave del otro.
