...
Este capítulo describe la materialización de la arquitectura propuesta en Fase~1 y Fase~2. Ambas reutilizan el activo Widevine y los servicios de medios del entorno experimental, pero modifican de forma sustancial el camino de acceso: Varnish dirige las operaciones al \textit{control-plane}, el origen y el servidor de licencias dejan de publicarse en el anfitrión y el cliente necesita una sesión activa para cargar el manifiesto o solicitar una licencia.

La exposición se centra en decisiones comprobables en el código y en la configuración desplegada. Los valores indicados corresponden al prototipo ejecutado; no se presentan como parámetros universales para un servicio OTT. Las pruebas comparativas, los códigos observados y el impacto de los controles se desarrollarán en el Capítulo~7.

\section{Despliegue con Docker}

\subsection{Contenedores}

Fase~1 se despliega mediante seis contenedores: Varnish, \textit{control-plane}, origen Nginx, servidor interno de licencias, cliente OTT y laboratorio de auditoría. Fase~2 conserva esos componentes y añade Redis, alcanzando siete servicios. Los clientes y laboratorios son aplicaciones estáticas servidas por Nginx; el plano de control y el proxy de licencia se implementan con Node.js y Express.
...
\texttt{audit-lab} & Reproducción controlada, inspección de claims y escenarios reproducibles. & 1 y 2 \\
\texttt{redis} & Sesiones, eventos, riesgo, contadores y bans sin persistencia en disco. & 2 \\
\bottomrule
\end{tabularx}
\end{table}

Las imágenes de Nginx y Redis proceden de versiones ligeras basadas en Alpine. Varnish utiliza la imagen 7.5 y reserva 256~MB para su almacenamiento en memoria. Los dos servicios Node.js se construyen desde los directorios de cada fase, lo que permite modificar su lógica sin mezclarla con la imagen de distribución.
...
\begin{tabularx}{\textwidth}{p{0.18\textwidth}p{0.19\textwidth}p{0.19\textwidth}Y}
\toprule
\textbf{Fase} & \textbf{Cliente} & \textbf{Laboratorio} & \textbf{CDN / API} \\
\midrule
Fase~1 & \texttt{localhost:9300} & \texttt{localhost:9301} & \texttt{localhost:9080} \\
Fase~2 & \texttt{localhost:9400} & \texttt{localhost:9401} & \texttt{localhost:9180} \\
\bottomrule
\end{tabularx}
\end{table}
...
Varnish responde a las preflight CORS y solo admite los orígenes de cliente y laboratorio de su fase. Las cabeceras autorizadas se limitan a autorización, tipo de contenido, rangos, sesión y dispositivo. En Fase~2 se expone además \texttt{X-Risk-Score}. Todas las respuestas del frontal se marcan como \texttt{no-store}, puesto que el prototipo prioriza la consistencia de las decisiones sobre una evaluación de caché.
...
La función de firma serializa un único objeto JSON, lo codifica como Base64URL y calcula un HMAC-SHA256. La verificación vuelve a calcular la firma y utiliza una comparación de tiempo constante antes de interpretar la carga. El formato resultante tiene dos segmentos y, como se ha indicado en el capítulo anterior, no es un JWT RFC~7519 completo.

El token de acceso contiene \texttt{typ}, \texttt{sub}, \texttt{accountId}, \texttt{deviceId}, plan, entitlements, \texttt{jti} y expiración. Fase~2 añade los roles. El token de reproducción sustituye plan y permisos generales por \texttt{assetId}, \texttt{sessionId} y los ámbitos \texttt{manifest:read}, \texttt{content:read}, \texttt{license:request} y \texttt{heartbeat:write}. Los secretos de acceso y reproducción son distintos para impedir que una credencial de un tipo pueda validarse con la clave del otro.
...
\subsection{Validación de IP, dispositivo, activo y sesión}

La implementación no realiza geolocalización. La IP fiable proporcionada por Varnish se utiliza para rate limiting en Fase~1 y para correlación temporal en Fase~2. El \texttt{deviceId} se genera en el navegador, se incluye en el login y queda ligado tanto al token como a la sesión. Cada validación compara cuenta, dispositivo y activo del token con el objeto de sesión almacenado.

El manifiesto contrasta también el \texttt{assetId} de la ruta, mientras que manifiesto, contenido y licencia comparan la cabecera \texttt{X-Playback-Session-Id}. Un activo diferente se rechaza como falta de autorización y una sesión diferente como conflicto. Esta distinción facilita el análisis posterior sin revelar el token utilizado.
...
La creación de sesión elimina primero las sesiones que han superado la gracia de heartbeat. Después comprueba el entitlement y cuenta las sesiones activas de la cuenta. Si no se supera el máximo, genera un UUID, almacena el contexto y emite el token de reproducción.
...
Fase~2 registra cada evento como JSON en una lista Redis limitada a 300 elementos. El productor añade UUID, tipo y marca temporal a los campos específicos de la operación. Entre los tipos implementados se encuentran \texttt{auth.success}, \texttt{auth.failure}, \texttt{auth.blocked}, \texttt{playback.session\\_created}, \texttt{playback.concurrency\\_rejected}, \texttt{manifest.request}, \texttt{content.request}, \texttt{license.request}, \texttt{playback.heartbeat}, \texttt{playback.stopped}, \texttt{playback.expired}, \texttt{risk.incremented}, \texttt{ban.created} y \texttt{ban.cleared}.
...
El cliente de Fase~2 añade un panel de observabilidad que consulta \texttt{/admin/overview}. La respuesta integra sesiones, score, razones, bans y los veinte eventos más recientes. Los botones administrativos crean o retiran un ban del dispositivo visible. La Figura~\ref{fig:fase2-cliente-observabilidad} muestra la reproducción legítima y la aparición de eventos de sesión, manifiesto y licencia; su análisis cuantitativo se reserva para la evaluación.
...
Con esta integración quedan implementadas las dos etapas de mejora: Fase~1 corrige los caminos de acceso y crea una sesión verificable; Fase~2 añade memoria del comportamiento y capacidad de bloqueo. El capítulo siguiente evaluará si estos mecanismos producen las respuestas esperadas frente a los escenarios de Fase~0, qué evidencias dejan y qué coste funcional introducen.
