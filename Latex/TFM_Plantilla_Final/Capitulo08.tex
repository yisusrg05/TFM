...
\begin{enumerate}
    ...
    \item \textbf{Autorización dinámica.} Se ha implementado un proxy protegido que valida identidad, \textit{entitlement}, activo, sesión, dispositivo e instancia antes de entregar manifiesto, contenido o licencia. El prototipo utiliza tokens firmados mediante HMAC con un formato simplificado de dos segmentos, por lo que no debe presentarse como una implementación JWT completa. La IP se ha empleado como señal de correlación y no se ha realizado geolocalización. El objetivo se ha alcanzado en su función de autorización contextual, aunque estas dos características concretas han quedado acotadas respecto a la formulación inicial.
...
La metodología incremental también se ha cumplido. Cada fase ha partido de la anterior, ha introducido controles identificables y ha sido sometida a una regresión. La incidencia encontrada en el binding de sesión de \texttt{/license} durante la campaña ha demostrado la utilidad de este enfoque: la primera ejecución detectó el defecto, se corrigió la validación y se repitieron los 65 casos antes de aceptar el resultado final.
...
Los identificadores de dispositivo e instancia se generan en el cliente y no equivalen a una atestación criptográfica de hardware. Un atacante con control suficiente sobre el cliente podría reproducir esos valores. Tampoco se han evaluado la robustez del CDM, HDCP, la seguridad del sistema operativo, la captura de pantalla o la extracción de la salida ya descifrada. Ningún control de acceso elimina por completo el denominado \textit{analog hole}.
