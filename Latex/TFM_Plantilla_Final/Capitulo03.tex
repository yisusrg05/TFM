...
El empaquetado local se realiza con Shaka Packager en un contenedor. A partir de un MP4 de entrada, los scripts del laboratorio generan una inicialización MP4, segmentos \nolinkurl{m4s} y un MPD DASH. El empaquetado no realiza \textit{transcoding}; el activo de entrada ya se encuentra codificado. Esta separación entre codificación y empaquetado coincide con el modelo de Shaka Packager \cite{shaka-packager-docs}.
...
Python se ha utilizado como herramienta auxiliar para el tratamiento de evidencias y métricas. Su biblioteca estándar y su ecosistema han facilitado automatizar la lectura de registros estructurados, normalizar marcas temporales, agrupar peticiones por cuenta o sesión y generar tablas de resultados \cite{python-docs2026}. No ha formado parte del camino crítico de reproducción implementado en los contenedores, que se ha apoyado principalmente en Node.js y JavaScript; su papel ha sido facilitar el análisis reproducible de PCAP, logs y métricas obtenidos durante la evaluación.
...
En este TFM, Wireshark se ha empleado para contrastar el flujo observable desde el cliente oficial con el generado por los clientes externos de Fase~0. El objetivo no ha sido inspeccionar material criptográfico interno del CDM, sino identificar qué URLs, cabeceras, tokens, MPD y endpoints de licencia han quedado expuestos para un usuario que observa sus propias comunicaciones.

\subsection{\textit{tcpdump}}

\textit{tcpdump} ha complementado a Wireshark como mecanismo de captura no interactiva. Se ha utilizado para obtener ficheros PCAP en puntos concretos de la red del laboratorio y conservar evidencias revisables posteriormente. La captura se ha acotado a los puertos y contenedores de interés, evitando recoger tráfico ajeno al experimento.

La combinación de \textit{tcpdump} para la adquisición y Wireshark para la exploración ha permitido separar el momento de la prueba del análisis posterior. Los PCAP se han tratado como evidencia de los escenarios reproducidos y no como fuente de claves DRM: el contenido de licencia Widevine puede viajar cifrado o en formato propietario y las claves no son exportadas por EME al código de la aplicación.

\subsection{Credenciales de acceso y reproducción}

Las fases endurecidas utilizan credenciales firmadas con atributos de acceso y reproducción. La implementación incluye tipo de credencial, cuenta, dispositivo, activo, identificador de sesión, ámbito y expiración. Se trata de un mecanismo deliberadamente reducido para el laboratorio, cuyo propósito es aportar contexto verificable a las peticiones y no reproducir un proveedor de identidad completo.
...
El laboratorio usa MPEG-DASH, cuyo manifiesto se denomina MPD (\textit{Media Presentation Description}), como formato de entrega. El MPD describe una representación de vídeo, su inicialización y la plantilla de segmentos que debe recuperar el reproductor. Para los activos empaquetados localmente, Shaka Packager genera un fichero de inicialización y segmentos \nolinkurl{m4s}; el cliente obtiene estos objetos según el MPD. Esta organización reproduce la estructura básica de una distribución adaptativa, aunque el activo de laboratorio se limita a una representación de baja resolución y no persigue evaluar algoritmos de adaptación de bitrate.
...
Los scripts de empaquetado reciben un MP4 local y ejecutan Shaka Packager dentro de un contenedor. En el modo CENC/Widevine de laboratorio se especifican un KID, una clave de prueba, el esquema \nolinkurl{cenc} y la señalización Widevine. El MPD resultante contiene elementos \nolinkurl{ContentProtection}, el KID por defecto y datos PSSH, que permiten al reproductor reconocer que el activo requiere un sistema de claves compatible.
...
El contenido local se monta en el contenedor de origen como volumen de solo lectura. En Fase~0, Varnish reescribe las rutas \nolinkurl{/content/*} hacia el origen y puede cachear objetos multimedia. La misma fase deja visibles los puertos del origen y del servicio de licencias para mostrar que una CDN sin controles de negocio no constituye una barrera suficiente.
...
La configuración Widevine del cliente asocia el sistema \nolinkurl{com.widevine.alpha} con el endpoint de licencia de la fase activa. En Fase~0, la plataforma oficial usa \nolinkurl{/platform/license}, mientras que la ruta \nolinkurl{/license/no_auth} se conserva para el cliente externo y la auditoría. En Fase~1 y Fase~2, el endpoint público siempre corresponde al \textit{control-plane}; el servidor que contacta con el servicio de pruebas solo es accesible desde la red Docker.
...
La adquisición de tráfico se ha realizado en dos niveles. El primero ha consistido en observar la comunicación desde el navegador durante una reproducción oficial y durante una reproducción externa de Fase~0. El segundo ha consistido en capturar el tráfico asociado a los contenedores o a los puertos publicados, acotando las capturas a HTTP, DASH y licencias. Esta estrategia ha permitido conservar PCAP comparables para cada escenario, con marcas temporales que relacionan la solicitud de manifiesto, la emisión de licencia y la descarga de segmentos.

El análisis de las capturas ha buscado evidencias de superficie expuesta: URL de MPD, endpoint de licencia, cabeceras de autorización, redirecciones, respuestas de error y secuencia de solicitudes. No se ha interpretado ningún PCAP como mecanismo para obtener claves de Widevine, ya que el alcance se ha limitado al comportamiento observable del sistema y a los datos de control transmitidos por la aplicación.
...
Las métricas se han seleccionado para responder a la hipótesis del trabajo y permitir una comparación entre fases. Se han registrado el resultado de la autorización de licencia, el número de sesiones activas por cuenta, el número de solicitudes de contenido y licencia, el tiempo de respuesta de los endpoints críticos y el número de accesos bloqueados. En Fase~2 se ha añadido la evolución de la puntuación de riesgo, las razones que la componen y el número de bloqueos automáticos o manuales.

Además de las métricas de seguridad, se ha observado el impacto operativo de las defensas: latencia adicional de la obtención de licencia, errores de reproducción atribuibles a la expiración del token y comportamiento ante una sesión detenida o bloqueada. La evaluación del Capítulo~7 ha comparado el sistema base, donde un cliente externo puede reutilizar una ruta sin autenticación, con los sistemas endurecidos, donde la licencia queda condicionada al contexto de reproducción. Esta comparativa ha permitido distinguir de manera cuantitativa entre disponer de DRM y gobernar efectivamente el acceso al servicio.

Para favorecer la trazabilidad y la reproducción de los experimentos, el código fuente, las configuraciones Docker, los laboratorios, la memoria y las evidencias del proyecto se encuentran disponibles en un repositorio público de GitHub.\footnote{Repositorio público del TFM: \url{https://github.com/yisusrg05/TFM}} El repositorio incluye además documentación específica para desplegar cada fase e interpretar los artefactos obtenidos durante la evaluación.
